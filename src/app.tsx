...
import "./styles/newLandingPage2.css";
import "./styles/magicToolsAnimations.css";
...
import FloatingDevControls from "./components/FloatingDevControls";
...
import LandingPageSwitcher from "./components/LandingPageSwitcher";
...
if (
  ...
  console.error = (...args) => {
    ...
    if (
      ...
      console.warn(
        "🟡 Expected restriction in Builder.io environment:",
        ...args,
      );
...
function AppContent() {
  ...
  React.useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      if (button && !user) {
        ...
        console.log("🔥 Button clicked by anonymous user:", buttonText);
        ...
        if (isGenerateButton) {
          console.log("🔥 GENERATE BUTTON DETECTED - showing popup");
          e.preventDefault();
          e.stopPropagation();
          setShowFreeCreditsPopup(true);
          return false;
  ...
  React.useEffect(() => {
    if (import.meta.env.DEV) {
      (window as any).enablePremiumFeatures = () => {
        ...
        console.log("🔧 Premium features enabled! Refreshing...");
        setTimeout(() => window.location.reload(), 100);
      };

      (window as any).disablePremiumFeatures = () => {
        localStorage.removeItem("dev_force_premium");
        console.log("🔧 Premium features disabled! Refreshing...");
        setTimeout(() => window.location.reload(), 100);
      };
      ...
      (window as any).testPremiumFeatures = () => {
        ...
        console.log("🔧 COMPREHENSIVE Premium Feature Debug:");
        console.log("==========================================");
      ...
      (window as any).fixPremiumFeatures = () => {
        console.log("🔧 Attempting to fix premium features...");
        ...
        if (user?.uid) {
          ...
          console.log("✅ Set subscription to Pro");
        }
      ...
      (window as any).unlockPremium = () => {
        console.log("🚨 EMERGENCY PREMIUM UNLOCK");
        localStorage.setItem("dev_force_premium", "true");
      ...
      console.log("🔧 Premium testing utilities available:");
      console.log(
  ...
  // Add navigation event listener for calendar reminders
  useEffect(() => {
    const handleNavigateToTab = (event: CustomEvent) => {
      const { tab } = event.detail;
      if (tab && typeof tab === 'string') {
        setActiveTab(tab as any);
        console.log(`🧭 Navigated to ${tab} tab via calendar reminder`);
      }
    };
  ...
  const handleSignOut = async () => {
    try {
      ...
      // Clear any stored state and force return to landing
      console.log("🔄 User signed out - returning to landing page");

      // Force a page refresh to ensure clean state and proper landing page display
      window.location.reload();
    } catch (error) {
  ...
  const handleSignInClick = () => {
    console.log("�� handleSignInClick called - setting showAuth to true");
    setShowAuth(true);
  };

  const handleStartCreating = () => {
    if (!user) {
      // If user is not authenticated, show auth modal for signup
      console.log("🎆 User not authenticated - showing auth modal for signup");
      setShowAuth(true);
    } else {
  ...
  if (!user && !showAuth && !showAppDirectly) {
    return <LandingPageSwitcher onSignInClick={handleSignInClick} onStartCreating={handleStartCreating} />;
  }
  ...
  return (
    ...
    >
                ...
                <div>
                  ...
                  <div className="flex items-center mt-1">
                    ...
                    >
                      {billingInfo?.subscription?.planId &&
                      billingInfo.subscription.planId !== "free"
                        ? `${billingInfo.subscription.planId.charAt(0).toUpperCase() + billingInfo.subscription.planId.slice(1)} Account 🔧`
                        : billingInfo?.status === "active"
                          ? "Pro Account"
                          : "Free Account"}
                    </span>
      ...
      {/* Main app content */}
      {currentPage === "billing" ? (
        <BillingPage onBackToApp={() => setCurrentPage("app")} />
      ) : currentPage === "account" ? (
        <div className="pt-20">
      ...
      {/* Floating Dev Controls - Development only */}
      <FloatingDevControls
        userPlan={billingInfo?.subscription?.planId || "free"}
      ...
      {/* AI Studio Assistant - Available globally */}
      <FloatingAIAssistant
        onNavigateToTab={(tab) => {
          // Handle navigation from AI assistant
          console.log(`AI Assistant navigation to: ${tab}`);

          // Map AI assistant tab names to actual app tab IDs
          const tabMapping: Record<string, string> = {
            'generator': 'generator',
            'canvas': 'canvas',
            'thumbnails': 'thumbnailMaker',
            'ytAnalysis': 'channelAnalysis',
          ...
          if (tab === 'billing') {
          ...
          } else if (mappedTab && typeof setActiveTab === 'function') {
            ...
            AppNotifications.custom(
              "Navigation",
              `Switched to ${tab} tool as requested by AI Assistant`,
              "success"
            );
          } else {
            console.warn(`Unknown tab requested by AI Assistant: ${tab}`);
            AppNotifications.custom(
              "Navigation Error",
              `Unknown tab: ${tab}. Please try a different tool.`,
              "warning"
            );
          }
        }}
        userPlan={billingInfo?.subscription?.planId || "free"}
        isVisible={user || showAppDirectly} // Only show for authenticated users or in demo mode
      />
