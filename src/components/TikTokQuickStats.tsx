...
interface TikTokQuickStatsProps {
  userPlan?: string;
  isPremium?: boolean;
  onUpgrade?: () => void;
  enabled?: boolean;
...
const TikTokQuickStats: React.FC<TikTokQuickStatsProps> = ({
  userPlan = "free",
  isPremium = false,
  onUpgrade,
  enabled = false,
  ...
  const [manualData, setManualData] = useState<Partial<TikTokStats>>({});
  ...
  // Mock TikTok stats
  const mockStats: TikTokStats = {
    followers: 127583,
    totalViews: 3945821,
    totalVideos: 89,
    avgViewsPerVideo: 44337,
    monthlyRevenue: 1847,
    monthlyViews: 582947,
    engagementRate: 12.4,
    followerGrowth: 18.6,
    viewGrowth: 24.1,
    revenueGrowth: 31.5,
    topVideoViews: 1247382,
    likesReceived: 284739,
    commentsReceived: 47382,
    sharesReceived: 18473,
    uploadFrequency: "2-3x/day",
    lastUpdated: new Date(),
  };

  const [stats, setStats] = useState<TikTokStats>(mockStats);
  ...
  useEffect(() => {
    const savedSettings = localStorage.getItem('studioHub:tiktokStatsSettings');
    if (savedSettings) {
  ...
  const saveSettings = () => {
    ...
    localStorage.setItem('studioHub:tiktokStatsSettings', JSON.stringify(settings));
  };
  ...
  const handleUploadData = (data: Partial<TikTokStats>) => {
    setManualData(data);
    setStats({ ...mockStats, ...data });
  ...
  if (!statsEnabled) {
    return (
      <Card className="bg-gradient-to-r from-pink-500/5 to-rose-600/5 border border-pink-500/20">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-500 to-rose-600 rounded-lg flex items-center justify-center opacity-50">
              <Music className="w-5 h-5 text-white" />
            </div>
            <div>
              <h3 className="font-medium text-[var(--text-primary)]">
                TikTok Analytics (Hidden)
              </h3>
              <p className="text-sm text-[var(--text-secondary)]">
                Click to show your TikTok dashboard
              </p>
            </div>
          </div>
          <Button
    ...
    <>
      <Card className="relative overflow-hidden">
            ...
            <div>
              <h2 className="text-xl font-bold text-[var(--text-primary)] flex items-center space-x-2">
                ...
                <Badge variant="neutral" className="text-xs">
                  {Object.values(metricSettings).filter(Boolean).length}/11 metrics
                </Badge>
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-orange-500/10 to-red-500/10 border border-orange-500/20 rounded-xl">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--text-secondary)]">
                TikTok doesn't offer an official public data API like YouTube. To use these analytics features,
                you'll need to manually upload your performance data from TikTok's Creator Fund dashboard
                or business account analytics.
              </p>
            </div>
          </div>
        </div>
        ...
        {metricSettings.overview && (
          <div className="bg-gradient-to-r from-pink-500/10 to-rose-600/10 border border-pink-500/20 rounded-lg p-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-semibold text-[var(--text-primary)]">
                    {dataSource === 'manual' ? "Data Updated" : "Upload Your TikTok Data"}
                  </h3>
                  <p className="text-sm text-[var(--text-secondary)]">
                    {dataSource === 'manual' 
                      ? "Last updated " + new Date(stats.lastUpdated).toLocaleTimeString()
                      : "Import analytics from TikTok Creator Fund or Business Account"
                    }
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-[var(--text-secondary)]">
                      Display TikTok analytics in your dashboard
                    </div>
                  </div>
                  <button
            ...
            >
              ...
              <div className="mb-6 p-4 bg-orange-500/10 border border-orange-500/20 rounded-xl">
                ...
                <ol className="text-sm text-[var(--text-secondary)] space-y-1">
                  <li>1. Go to TikTok Creator Fund dashboard</li>
                  <li>2. Navigate to Analytics section</li>
                  <li>3. Copy the numbers from your overview</li>
                  <li>4. Enter them in the form below</li>
                </ol>
              </div>
...
export default TikTokQuickStats;
